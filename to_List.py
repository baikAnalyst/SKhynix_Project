...
df = pd.read_csv(r"D:\BAIK\DATA.STUDY\SeSAC\02.jongro\SKhynix_Project\SKhynix_Project\SKhynix_Project\sk_data_del.csv")
...
for i, row in df.iterrows():
  try:
    morphs = mecab.parse(row['contents'])
    tokens = []
    pos_tag = []
    lines = morphs.split("\n")
    for line in lines:
...
topic_list = []  # Initialize an empty list to store the topics

for t in lda.print_topics(num_topics=n):
    topic_list.append(t)

# Now, topic_list contains the topics as strings
print(topic_list)

dff = to_DataFrame('topic_list')
